  ...
  methods: {
    ...
    async validateExifData (file) {
      ...
      if (!exif.tags.CreateDate) {
        throw new Error(`Photo file (${file.name}) is missing timestamp (CreateDate) in EXIF data`)
      }
    ...
    async selectUtcOffset () {
      ...
      const rawTimestamp = new Date(exif.tags.CreateDate * 1000)
      ...
      this.timestamp.verify = {
        ...
        localTimestamp: timestamp.local().tz(this.station.timezone).format('MMM D, YYYY h:mm:ss a z'),
