...
const { File, Organization, Station } = require('../db/models')
...
async function processFile (id) {
  ...
  // create series
  const series = await station.$relatedQuery('series').insertGraph({
    file_id: file.id,
    values
  })
...
module.exports = async function (id, { dryRun }) {
  try {
    const file = await processFile(id, dryRun)
    console.log(`file processed successfully (id=${file.id}, filename=${file.filename})`)
    // console.log(file)
  } catch (e) {
    console.log(`failed to process file: ${e.message || e.toString()}`)
